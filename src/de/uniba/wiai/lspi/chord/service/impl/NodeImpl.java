...
//			 add potential predecessor to successor list and finger table and
	...
	public final void broadcast(Broadcast info) throws CommunicationException {
		...
		//sende an alle EintrÃ¤ge in der Fingertable die zwischen meiner ID und Range sind
		Node node = null;
		...
		for (int i = 0; i < fingertable.size(); i++) {
			...
			if(node.getNodeID().isInInterval(this.getNodeID(), range)) {
				...
				// Wenn der nÃ¤chste Finger-table-Eintrag grÃ¶ÃŸer ist als die Range wird die initiale range genutzt
